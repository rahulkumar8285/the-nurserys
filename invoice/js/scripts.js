...
$(document).ready(function() {
	...
	// add product
	$("#action_add_quotation").click(function(e) {
		e.preventDefault();
	    actionAddquotation();
	});
	...
	//// update Quotation
	$(document).on('click', "#action_update_quotation", function(e) {
		e.preventDefault();
		updateQuotation();
	});
	...
	function actionAddquotation() {
		...
		if (errorCounter > 0) {
			...
			var $btn = $("#action_add_quotation").button("loading");

			$.ajax({

				url: 'response.php',
				type: 'POST',
				data: $("#add_quotation").serialize(),
				dataType: 'json',
				success: function(data){
					$("#response .message").html("<strong>" + data.status + "</strong>: " + data.message);
					$("#response").removeClass("alert-warning").addClass("alert-success").fadeIn();
	...
	function updateQuotation() {
		var $btn = $("#action_update_quotation").button("loading");
		jQuery.ajax({

			url: 'response.php',
			type: 'POST', 
			data: $("#update_quotation").serialize(),
			dataType: 'json', 
			success: function(data){
				$("#response .message").html("<strong>" + data.status + "</strong>: " + data.message);
				$("#response").removeClass("alert-warning").addClass("alert-success").fadeIn();
